(() => {
  ...
  function schedule(){
    ...
    if(!paused&&!document.hidden)timer=setTimeout(()=>show(current+1),duration);
  }
  ...
  root.querySelector('[data-carousel-prev]').addEventListener('click',()=>{show(current-1,true);});
  root.querySelector('[data-carousel-next]').addEventListener('click',()=>{show(current+1,true);});
  ...
  root.addEventListener('pointerup',e=>{if(!start)return;const x=e.clientX-start.x,y=e.clientY-start.y;start=null;if(Math.abs(x)>55&&Math.abs(x)>Math.abs(y)*1.5){suppressClick=true;show(current+(x<0?1:-1),true);setTimeout(()=>suppressClick=false,400);}});
